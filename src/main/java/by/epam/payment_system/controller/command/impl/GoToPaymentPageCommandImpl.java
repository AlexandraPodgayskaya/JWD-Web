...
public class GoToPaymentPageCommandImpl implements Command {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		session.setAttribute(ParameterConstraint.PAGE,
				GoToPage.PAYMENT_PAGE + URIConstructor.SET_NUMBER_CARD + request.getParameter(ParameterConstraint.NUMBER_CARD)
						+ URIConstructor.SET_CURRENCY + request.getParameter(ParameterConstraint.CURRENCY)
						+ URIConstructor.SET_BALANCE + request.getParameter(ParameterConstraint.BALANCE));
